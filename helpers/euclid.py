...
class Result:
    def __init__(self, nod, iterations):
        self.nod = nod
...
def binary_euclid(a: int, b: int, nod: int):
    ...
    factoring_log = ''
    if a != init_a:
        factoring_log += f'{init_a} / 2^{power_a} -> {a}\n' if power_a != 1 else f'{init_a} / 2 -> {a}\n'
        nod_changed = True
    if b != init_b:
        factoring_log += f'{init_b} / 2^{power_b} -> {b}\n' if power_b != 1 else f'{init_b} / 2 -> {b}\n'
        nod_changed = True
    if nod_changed:
...
def euclid(a: int, b: int, binary=True, comment=False) -> int:
    ...
    if nod == 1:
        pretty_print(f'  gcd({init_a}, {init_b}) = {nod}, числа взаимно-простые', grid='fancy_grid')
    else:
        pretty_print(f'  gcd({init_a}, {init_b}) = {nod}, числа не взаимно-простые', grid='fancy_grid')
